		...
		*/
		...
		function initialize() {
			...
			add_filter( 'frontend_admin/forms/get_form', function( $form, $key ){
				$key = explode( 'rel_', $key );				
				
				if( ! isset( $key[1] ) || empty( $_POST['_acf_ajax_args'] ) ) return $form;
				
				$args = wp_parse_args(
					json_decode( $_POST['_acf_ajax_args'], true ),
					array(
						'field_key'   => $key[1],
						'parent_form' => '',
						'form_action' => '',
		...
		/**
		 * Load the ajax form for this field
		 *
		 * @param array $args
		 * @return array
		 */
		public function render_form( $args ) {
			$form = $this->get_form( $args );
			fea_instance()->form_display->render_form( $form );
			die;
		}

		public function get_field_form( $parent ){
			if( 'admin_form' == get_post_type( $parent ) || 'acf-field-group' == get_post_type( $parent ) ){
				return $parent;
			}

			return $this->get_field_form( get_post_field( 'post_parent', $parent ) );
		}

		/**
		 * Get the form for this field
		 *
		 * @param array $args
		 * @return array
		 */
		public function get_form( $args ) {
			...
			if ( ! empty( $field['post_form_template'] ) ) {
				...
				if ( in_array( 'current', $chosen ) ) {
					...
					$chosen[ $pos ] = $this->get_field_form( $field['parent'] );
				...
				foreach ( $chosen as $chosen_fields ) {
					...
					$source = $type_of_choice[1];

					switch ( $type_of_choice[0] ) {
						case 'field':
							$current_field = acf_get_field( $source );
							if ( $current_field ) {
								$form_args['fields'][] = $current_field;
							}
							break;
						case 'group':
							$form_args['fields'] = array_merge( $form_args['fields'], acf_get_fields( $source ) );
							break;
						case 'form':
							$current_group = fea_instance()->form_display->get_form_fields( $source );
	...
	<div class="selection">
		...
		*/
		...
		public function edit_post_button( $title, $post, $field, $post_id ) {
			if( 'post_object' == $field['type'] ) return $title;
			
			if ( $post && ! empty( $field['add_edit_post'] ) ) :
